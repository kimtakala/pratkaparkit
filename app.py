...
def create_spot():
    ...
    if request.method == "POST":
        ...
        items.create_spot(current_user_id(), data["title"], data["description"], data["lat"], data["lon"], data["address"], data["tags"])
...
def spot_detail(spot_id):
    ...
    return render_template("spot_detail.html", spot=spot, comments=comments_list, csrf_token=csrf_token, current_user_id=current_user_id(), tile_url=tile_url)
...
def edit_spot(spot_id):
    ...
    if request.method == "POST":
        ...
        items.update_spot(spot_id, data["title"], data["description"], data["lat"], data["lon"], data["address"], data["tags"])
...
def user_profile(user_id):
    ...
    if not user:
        abort(404)
    spots = items.get_spots_by_owner(user_id)
