...
def get_comments_for_spot(spot_id):
    return query_all(
        "SELECT c.id, c.parking_spot_id, c.author_id, c.text, c.created_at, u.username AS author_name FROM comment c JOIN users u ON c.author_id = u.id WHERE c.parking_spot_id = ? ORDER BY c.created_at DESC",
        (spot_id,),
    )
